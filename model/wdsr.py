...
def wdsr(scale, num_filters, num_res_blocks, res_block_expansion, res_block_scaling, res_block, attention=False):
    ...
    if attention:
        ...
        m = ChannelAttention(num_filters, 3)(m)
        m = SpatialAttention(8)(m)
    else:
        m = conv2d_weightnorm(num_filters, 3, padding='same')(x)

    for i in range(num_res_blocks):
        m = res_block(m, num_filters, res_block_expansion, kernel_size=3, scaling=res_block_scaling, attention=attention)

    if attention:
        m = conv2d_weightnorm(3 * scale ** 2, 3, padding='same', name=f'conv2d_main_scale_{scale}')(m)
        m = ChannelAttention(3 * scale ** 2, 3)(m)
        m = SpatialAttention(8)(m)
    else:
        m = conv2d_weightnorm(3 * scale ** 2, 3, padding='same', name=f'conv2d_main_scale_{scale}')(m)
    ...
    if attention:
        ...
        s = ChannelAttention(3 * scale ** 2, 3)(s)
        s = SpatialAttention(8)(s)
    else:   
        s = conv2d_weightnorm(3 * scale ** 2, 5, padding='same', name=f'conv2d_skip_scale_{scale}')(x)
...
def res_block_b(x_in, num_filters, expansion, kernel_size, scaling, attention=False):
    linear = 0.8
